    ...
    User.find_each do |user|
      ...
      User.find_each do |another_user|
        ...
        if similarity_users.length > 10
          similarity_users.sort! { |a, b| a[1] <=> b[1] }
          similarity_users.shift
        end
      end
    
      similarity_users_liked_reviews = similarity_users.map { |a| a[0].liked_reviews }.flatten.uniq { |review| review.id }
